        ...
        mediaObject[prop].forEach(media => {
            if(prop === 'video') {
                const videoPromise = new Promise((resolve, reject) => {
                    cloudinary.v2.uploader.upload(media.path, {folder: NODE_ENV == 'develop' ? IMAGE_FOLDER_DEV : IMAGE_FOLDER, resource_type: 'video'}, function(error,result) {
                        if(error) {
                ...
                uploadPromises.push(videoPromise);
                  ...
                  const uploadPromise = new Promise((resolve, reject) => {
                      cloudinary.v2.uploader.upload(media.path, {folder: NODE_ENV == 'develop' ? IMAGE_FOLDER_DEV : IMAGE_FOLDER}, function(error,result) {
                          if(error) {
                              reject(error.message);
                          }
                          if(prop === 'hero') {
